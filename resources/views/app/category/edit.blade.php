...
@php
    $topbarTitle = __('Edit Categories');
@endphp
    ...
    <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
        ...
        <nav class="mb-6">
            <ol class="flex space-x-2 text-sm text-gray-600 dark:text-gray-400">
                <li>
                    <a href="{{ route('categories.index') }}" class="hover:text-gray-900 dark:hover:text-white">
                        {{ __('Categories') }}
                    </a>
                </li>
                <li class="text-gray-400">/</li>
                <li>
                    <a href="{{ route('categories.show', $item) }}" class="hover:text-gray-900 dark:hover:text-white">
                        {{ $item->name ?? $item->id }}
                    </a>
                </li>
                <li class="text-gray-400">/</li>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-4 gap-6">
            ...
            <div class="lg:col-span-3">
                <div class="bg-white dark:bg-gray-800 shadow-md rounded-lg p-6">
                    <form method="POST" action="{{ route('categories.update', $item) }}" class="space-y-4" id="editForm" onsubmit="handleEditFormSubmit(event)">
                        @csrf
                        @method('PUT')
...
<div id="deleteModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg max-w-sm w-full mx-4">
        ...
        <div class="px-6 py-4 border-t border-gray-200 dark:border-gray-700 flex gap-3 justify-end">
            ...
            <form method="POST" action="{{ route('categories.destroy', $item) }}" class="inline" id="deleteForm" onsubmit="handleDeleteSubmit(event)">
                @csrf
                @method('DELETE')
                <button type="submit"
