...
@php
    $topbarTitle = __('Create Customers');
@endphp
...
<div class="py-12">
    <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="bg-white dark:bg-gray-800 shadow-md rounded-lg p-6">
            <form method="POST" action="{{ route('customers.store') }}" class="space-y-4" id="createForm" onsubmit="handleFormSubmit(event)">
                @csrf
        ...
        <div class="mb-4">
          ...
          <input type="tel" id="phone" name="phone" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white" value="{{ isset($item) ? $item->phone : old('phone') }}" required @if(isset($isView)) disabled @endif />
          @error('phone')
            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
          @enderror
        </div>
        <div class="mb-4">
          ...
          <input type="email" id="email" name="email" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white" value="{{ isset($item) ? $item->email : old('email') }}" required @if(isset($isView)) disabled @endif />
          @error('email')
            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
          @enderror
        </div>
