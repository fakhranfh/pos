...
<div class="py-12">
    <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-4 gap-6">
            ...
            <div class="lg:col-span-3">
                <div class="bg-white dark:bg-gray-800 shadow-md rounded-lg p-6">
                    <form method="POST" action="{{ route('products.update', $item) }}" class="space-y-4" id="editForm" onsubmit="handleEditFormSubmit(event)">
                        @csrf
                        @method('PUT')
        ...
        <div class="mb-4">
          ...
          <input type="text" id="image_url" name="image_url" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white" value="{{ isset($item) ? $item->image_url : old('image_url') }}" required />
          @error('image_url')
            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
          @enderror
        </div>
