...
@section('title', __('Edit Stock Movements'))

@php
    $topbarTitle = __('Edit Stock Movements');
@endphp
    ...
    <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
        ...
        <nav class="mb-6">
            <ol class="flex space-x-2 text-sm text-gray-600 dark:text-gray-400">
                ...
                <li>
                    <a href="{{ route('stock-movements.show', $item) }}" class="hover:text-gray-900 dark:hover:text-white">
                        {{ $item->type ?? $item->id }}
                    </a>
                </li>
                <li class="text-gray-400">/</li>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-4 gap-6">
            ...
            <div class="lg:col-span-3">
                <div class="bg-white dark:bg-gray-800 shadow-md rounded-lg p-6">
                    <form method="POST" action="{{ route('stock-movements.update', $item) }}" class="space-y-4" id="editForm" onsubmit="handleEditFormSubmit(event)">
                        @csrf
                        @method('PUT')
        ...
        <div class="mb-4">
          ...
          <select id="type" name="type" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white" required>
            <option value="">-- @lang('Select') --</option>
            <option value="sale" {{ old('type', $item->type) === 'sale' ? 'selected' : '' }}>@lang('Sale')</option>
            <option value="stock_in" {{ old('type', $item->type) === 'stock_in' ? 'selected' : '' }}>@lang('Stock In')</option>
            <option value="adjustment" {{ old('type', $item->type) === 'adjustment' ? 'selected' : '' }}>@lang('Adjustment')</option>
            <option value="return" {{ old('type', $item->type) === 'return' ? 'selected' : '' }}>@lang('Return')</option>
          </select>
          @error('type')
            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
          @enderror
        ...
        <div class="mb-4">
          ...
          <input type="number" id="quantity_change" name="quantity_change" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white" value="{{ isset($item) ? $item->quantity_change : old('quantity_change') }}" required />
        ...
        <div class="mb-4">
          ...
          <textarea id="reason" name="reason" rows="4" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white" required>{{ isset($item) ? $item->reason : old('reason') }}</textarea>
          @error('reason')
            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
          @enderror
        </div>
        <div class="mb-4">
          ...
          <select id="user_id" name="user_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white" required>
            <option value="">-- @lang('Select') --</option>
...
<div id="deleteModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg max-w-sm w-full mx-4">
        ...
        <div class="px-6 py-4 border-t border-gray-200 dark:border-gray-700 flex gap-3 justify-end">
            ...
            <form method="POST" action="{{ route('stock-movements.destroy', $item) }}" class="inline" id="deleteForm" onsubmit="handleDeleteSubmit(event)">
                @csrf
                @method('DELETE')
                <button type="submit"
