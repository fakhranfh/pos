...
@section('title', __('Edit Transaction Items'))

@php
    $topbarTitle = __('Edit Transaction Items');
@endphp
    ...
    <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
        ...
        <nav class="mb-6">
            <ol class="flex space-x-2 text-sm text-gray-600 dark:text-gray-400">
                ...
                <li>
                    <a href="{{ route('transaction-items.show', $item) }}" class="hover:text-gray-900 dark:hover:text-white">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-4 gap-6">
            ...
            <div class="lg:col-span-3">
                <div class="bg-white dark:bg-gray-800 shadow-md rounded-lg p-6">
                    <form method="POST" action="{{ route('transaction-items.update', $item) }}" class="space-y-4" id="editForm" onsubmit="handleEditFormSubmit(event)">
                        @csrf
                        @method('PUT')
        ...
        <div class="mb-4">
          ...
          <select id="transaction_id" name="transaction_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white" required>
        ...
        <div class="mb-4">
          ...
          <select id="product_id" name="product_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white" required>
            <option value="">-- @lang('Select') --</option>
        ...
        <div class="mb-4">
          ...
          <input type="text" id="product_name" name="product_name" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white" value="{{ isset($item) ? $item->product_name : old('product_name') }}" required />
          @error('product_name')
            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
          @enderror
        </div>
        <div class="mb-4">
          ...
          <input type="number" id="unit_price" name="unit_price" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white" value="{{ isset($item) ? $item->unit_price : old('unit_price') }}" required />
          @error('unit_price')
            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
          @enderror
        </div>
        <div class="mb-4">
          ...
          <input type="number" id="quantity" name="quantity" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white" value="{{ isset($item) ? $item->quantity : old('quantity') }}" required />
          @error('quantity')
            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
          @enderror
        </div>
        ...
        <div class="mb-4">
          ...
          <input type="number" id="line_total" name="line_total" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white" value="{{ isset($item) ? $item->line_total : old('line_total') }}" required />
          @error('line_total')
            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
          @enderror
        </div>
...
<div id="deleteModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg max-w-sm w-full mx-4">
        ...
        <div class="px-6 py-4 border-t border-gray-200 dark:border-gray-700 flex gap-3 justify-end">
            ...
            <form method="POST" action="{{ route('transaction-items.destroy', $item) }}" class="inline" id="deleteForm" onsubmit="handleDeleteSubmit(event)">
                @csrf
                @method('DELETE')
                <button type="submit"
