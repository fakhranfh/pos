...
@section('title', $item->transaction_id ?? __('View Transaction Items'))

@php
    $topbarTitle = $item->transaction_id ?? __('View Transaction Items');
@endphp
    ...
    <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
        ...
        <nav class="mb-6">
            <ol class="flex space-x-2 text-sm text-gray-600 dark:text-gray-400">
                ...
                <li class="text-gray-900 dark:text-white">{{ $item->product_name ?? $item->id }}</li>
            </ol>
        </nav>
        ...
        <div class="mb-6">
            <h1 class="text-3xl font-bold text-gray-900 dark:text-white">
                {{ $item->transaction_id ?? __('View Transaction Items') }}
            </h1>
        ...
        <div class="border-t border-gray-200 dark:border-gray-700 py-4">
          <dt class="text-sm font-medium text-gray-600 dark:text-gray-400">@lang('Transaction Id')</dt>
        ...
        <div class="border-t border-gray-200 dark:border-gray-700 py-4">
          <dt class="text-sm font-medium text-gray-600 dark:text-gray-400">@lang('Product Name')</dt>
          <dd class="mt-1 text-sm text-gray-900 dark:text-white">
            {{ $item->product_name }}
          </dd>
        </div>
        <div class="border-t border-gray-200 dark:border-gray-700 py-4">
          <dt class="text-sm font-medium text-gray-600 dark:text-gray-400">@lang('Unit Price')</dt>
          <dd class="mt-1 text-sm text-gray-900 dark:text-white">
            {{ $item->unit_price }}
          </dd>
        </div>
        <div class="border-t border-gray-200 dark:border-gray-700 py-4">
          <dt class="text-sm font-medium text-gray-600 dark:text-gray-400">@lang('Quantity')</dt>
          <dd class="mt-1 text-sm text-gray-900 dark:text-white">
            {{ $item->quantity }}
          </dd>
        </div>
        ...
        <div class="border-t border-gray-200 dark:border-gray-700 py-4">
          <dt class="text-sm font-medium text-gray-600 dark:text-gray-400">@lang('Line Total')</dt>
          <dd class="mt-1 text-sm text-gray-900 dark:text-white">
            {{ $item->line_total }}
          </dd>
        </div>
            ...
            <div class="lg:col-span-1">
                <div class="bg-white dark:bg-gray-800 shadow-md rounded-lg p-6">
                    ...
                    <div class="space-y-2">
                        <a href="{{ route('transaction-items.edit', $item) }}"
...
<div id="deleteModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg max-w-sm w-full mx-4">
        ...
        <div class="px-6 py-4 border-t border-gray-200 dark:border-gray-700 flex gap-3 justify-end">
            ...
            <form method="POST" action="{{ route('transaction-items.destroy', $item) }}" class="inline">
                @csrf
                @method('DELETE')
                <button type="submit"
