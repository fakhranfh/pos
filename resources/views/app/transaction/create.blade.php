    ...
    <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="mb-4">
          ...
          <select id="status" name="status" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white" required @if(isset($isView)) disabled @endif>
            <option value="">-- @lang('Select') --</option>
            <option value="completed" {{ old('status', isset($item) ? $item->status?->value : '') === 'completed' ? 'selected' : '' }}>@lang('Completed')</option>
            <option value="voided" {{ old('status', isset($item) ? $item->status?->value : '') === 'voided' ? 'selected' : '' }}>@lang('Voided')</option>
          </select>
          @error('status')
            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
          @enderror
